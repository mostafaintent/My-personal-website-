-- این فایل رو بعد از migration های قبلی (schema.sql و بقیه) توی
-- Supabase Dashboard -> SQL Editor اجرا کنید.
--
-- این migration کاملاً افزایشی (additive) است: فقط ۴ جدول جدید و یک تابع
-- می‌سازه، هیچ جدول/داده‌ی موجودی رو تغییر، حذف، یا reset نمی‌کنه.
--
-- چهار جدول تعامل کاربر با مقاله:
--   article_favorites  — علاقه‌مندی (ذخیره برای بعد)
--   article_likes      — لایک (واکنش به محتوا)
--   article_views       — آخرین زمان بازدید هر کاربر از هر مقاله
--   article_reads       — زمانی که کاربر مقاله رو تا انتها خونده
--
-- هر چهارتا unique(user_id, article_id) دارن که رکورد تکراری ساخته نشه،
-- و RLS طوری تنظیم شده که هر کاربر فقط ردیف‌های خودش رو ببینه/بسازه/حذف کنه.
...
-- بالا فقط اجازه‌ی دیدنِ ردیف‌های خودِ کاربر رو می‌ده — یعنی هیچ کوئری
-- معمولی نمی‌تونه لیست کنه چه کسی لایک کرده، حتی خودِ شمارش کل هم از
-- طریق select عادی به‌درستی در نمی‌اومد (چون RLS ردیف‌های بقیه رو فیلتر
-- می‌کنه). برای همین یک تابع SECURITY DEFINER می‌سازیم که فقط یک عدد
-- (COUNT) برمی‌گردونه — نه ردیف‌ها، نه user_id — و اجازه‌ی اجراش رو به
-- کاربر مهمان (anon) و لاگین‌کرده (authenticated) می‌دیم.
...
-- ============ article_reads (مقالات خوانده‌شده) ============
